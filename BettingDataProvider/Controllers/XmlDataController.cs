...
namespace BettingDataProvider.Controllers
{
    public class XmlDataController : Controller
    {
        ...
        public XmlDataController(ApplicationDbContext context)
        {
            _context = context;
        }
                ...
                if (reader.NodeType == XmlNodeType.Element && reader.Name == "Match")
                {
                    ...
                    if (_context.Matches.Any(m => m.MatchId == matchID))
                    {
                        ...
                        if (matchFromDb.StartDate != matchStartDate)
                        {
                            ...
                            QueueManager.AddItem($"Match {matchFromDb.Name}'s start date was updated to {matchStartDate}");
                        }
                        if (matchFromDb.Type != matchMatchType)
                        {
                            ...
                            QueueManager.AddItem($"Match {matchFromDb.Name}'s type was updated to {matchType}");
                        }
                ...
                if (reader.NodeType == XmlNodeType.Element && reader.Name == "Odd")
                {
                    ...
                    if (_context.Odds.Any(o => o.OddId == oddId))
                    {
                        ...
                        if (oddFromDb.Value != oddValue)
                        {
                            ...
                            QueueManager.AddItem($"Odd {oddFromDb.Name}'s value was updated to {oddValue}");
                        }
                    }
                    else
                    {
